...
//{
    ...
    public class BarCodeHelper
    {
        ...
        public static Image Create128Code(string InputData, int FontSize, float BarWeight, int  BarHeight, int Margin_Top, int Margin_Left)
        {
            byte[] AsciiBytes = Encoding.ASCII.GetBytes(InputData);
            int[] CodeNumber = GetCodeNumber(AsciiBytes);

            float Width =Convert.ToInt32( CodeNumber.Length * 11 * BarWeight + 2 + Margin_Left * 2);
            float Height =Convert.ToInt32( BarHeight + FontSize + 5 + Margin_Top * 2);
            int width = Convert.ToInt32(Width);
            int height = Convert.ToInt32(Height)+5;
            Image BarCodeImg = new Bitmap(width, height);
            using (Graphics gr = Graphics.FromImage(BarCodeImg))
            {
                // 先画一块白布
                ...
                float barHeight = (float)BarHeight;
                foreach (int Item in CodeNumber)
                {
                    for (int i = 0; i < 8; i += 2)
                    {
                        float Black = Code128[Item, i] * BarWeight;
                        float White = Code128[Item, i + 1] * BarWeight;
                        //绘制黑色线条
                        if (Black > 0)
                        {
                            gr.FillRectangle(System.Drawing.Brushes.Black, Cursor, margin_Top, Black, BarHeight);
