...
namespace XinYaPCClientForAutoLoading
{
    public partial class frm_Main : Form
    {
        ...
        /// 当前工位对象
        /// </summary>
        private TB_SecondWorkStationInfo secondWorkStationInfo;
        ...
        /// 异常列表
        /// </summary>
        private List<TB_Exception> exceptions;

        /// <summary>
        /// 主任务
        /// </summary>
        private TB_WorkMain workMain;

        /// <summary>
        /// 子任务
        /// </summary>
        private List<TB_WorkDtl> workDtl;

        /// <summary>
        /// 客户端服务
        /// </summary>
        private ServiceReference1.Service1Client service1Client = new ServiceReference1.Service1Client();

        /// <summary>
        /// 当前工位类别
        /// </summary>
        private string stationCode ="";// System.Configuration.ConfigurationSettings.AppSettings["WorkStationCode"];

        /// <summary>
        /// 当前工位ID
        /// </summary>
        private int secondWorkStationId =0;// Convert.ToInt32(System.Configuration.ConfigurationSettings.AppSettings["SecondWorkStationID"]);
        ...
        /// 日志类
        /// </summary>
        private LogExecute log = new LogExecute();

        /// <summary>
        /// 用于承载从服务端返回的托盘编码
        /// </summary>
        private string[] str_PalletCodes = null;
        ...
        /// <summary>
        /// 托盘哨兵,初始值为0000，当当前没有托盘的时候初始值就是0
        /// </summary>
        private string palletGuard = "0000";
        ...
        public frm_Main(TB_User user)
        {
            ...
            //加载按钮初始不可用
            btn_Station1.Enabled = false;
            btn_Station2.Enabled = false;
            ...
            //是否非调试QC
            if (secondWorkStationInfo.WorkStationCode != "E" && secondWorkStationInfo.WorkStationCode != "F")
            {
                gb_WorkStation.Visible = false;
            }
            else
            {
                gb_WorkStation.Visible = true;
            }
            //如果是装后盖的话，不应该出现故障录入等信息
            if (secondWorkStationInfo.WorkStationCode == "C")
            {
                btn_AddException.Visible = false;
                btn_ClearException.Visible = false;
            }
        ...
        //    #region 当前工位workDtlForEachStation处理(完成)
        //    if (workMain != null)
        //    {
        ...
        //                    //    if (exception)
        //                    //    {
        //                    //        workDtlForEachStation.Remark = "不合格";
        //                    //    }
        //                    //    else
        //                    //    {
        //                    //        workDtlForEachStation.Remark = "合格";
        //                    //    }
        //                    //}
        ...
        //        {
        //            //根据托盘条码找任务
        //            workMain = db.TB_WorkMain.First(p => p.PalletCode == txt_PalletCode.Text && p.Finished == "0");
        //        }
        //        catch (Exception)
        //        {
        //            workMain = null;
        ...
        //        //else
        //        //{
        //        //    workDtl = workMain.TB_WorkDtl.Where(p => p.IsException == false).ToList();
        //        //}
        ...
        //        catch (Exception ex)
        //        {
        //            lab_CurrentStatus.Text = "当前主任务没有子任务存在！";
        ...
        /// <summary>
        /// 数据加载 flag表示要加载的托盘编码索引。0表示string[0],1表示string[1]
        /// </summary>
        /// <param name="flag"></param>
        public void DataInit(int flag)
        {
            if (flag != 0 && flag != 1)
            {
                return;
            ...
            using (XinYaDBEntities db = new XinYaDBEntities())
            {
                ...
                try
                {
                    string temp = str_PalletCodes[flag];
                    //根据托盘条码找任务
                ...
                {
                    workMain = null;
                    LogExecute.WriteLineDataLog("未找到" + str_PalletCodes[flag] + "对应的任务！");
                    lab_CurrentStatus.Text="未找到托盘" + str_PalletCodes[flag] + "编码对应的任务数据！改条码对应的任务未生成或已经完成。请核对！";
                    p_probar.Visible = false;
                    db.Dispose();
                    return;
                }
            ...
            if (mat != null)
            {
                if (mat.Class == "VE")
                {
                    ...
                    try
                    {
                        ...
                        //20150124新增图片展示页
                        pb_PhotoShow.Image = img;
                    }
                    catch (Exception)
                    {
                        pb_BengPicture.Image = null;
                        //20150124新增图片展示页
                        pb_PhotoShow.Image = null;
                    }
                    //"子表"
                    //气压
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btn_Shangyiye2_Click(object sender, EventArgs e)
        {
            tabControl1.SelectedTab = tabControl1.TabPages[2];
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btn_Xiayiye2_Click(object sender, EventArgs e)
        {
            tabControl1.SelectedTab = tabControl1.TabPages[3];
        }

        #endregion 翻页与dgvformatting事件
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void timer_GetPalletCodeFromPLC_Tick(object sender, EventArgs e)
        {
            ...
            if (secondWorkStationInfo.WorkStationCode == "E" || secondWorkStationInfo.WorkStationCode == "F")
            {
                ...
                //也就是说只要为null或者其中有一个工作台没有托偶盘编码，进入处理
                //if (str_PalletCodes == null || str_PalletCodes[0] == "0000" || str_PalletCodes[1] == "0000" || str_PalletCodes[0] == "0" || str_PalletCodes[1] == "0")
                //{
                    try
                    {
                        ...
                        //begin = DateTime.Now;
                        ////循环调用接口
                        //str_PalletCodes = service1Client.GetPalletCodesBySecondWorkStationID(secondWorkStationId);
                        //end = DateTime.Now;
                        //a = end - begin;
                        ////}
                        //a.ToString();
                        ...
                        //改为读取数据库
                        str_PalletCodes = GetPalletCodeBySQL();
                    }
                    catch (Exception)
                    {
                        //lab_CurrentStatus.Text = DateTime.Now.ToShortTimeString() + "远程服务未打开！";
                        lab_CurrentStatus.Text = DateTime.Now.ToShortTimeString()+"侦听托盘到位情况出现异常。";
                        return;
                    }
                    if (str_PalletCodes == null)
                    {
                        timer_GetPalletCodeFromPLC.Enabled = false;
                        //lab_CurrentStatus.Text = DateTime.Now.ToShortTimeString() + "远程服务未返回任何值！";
                        lab_CurrentStatus.Text = DateTime.Now.ToShortTimeString() + "侦听托盘到位情况出现异常！";
                        timer_GetPalletCodeFromPLC.Enabled = true;
                    }
                    else if (str_PalletCodes[0] == "0" || str_PalletCodes[1] == "0")//返回0表示PLC地址读取失败
                    {
                        lab_buhegebianma.Text = DateTime.Now.ToString() + "检查到上位机读取地址数据异常";
                    }
                    else
                    {

                        //按钮的可用性
                        if (str_PalletCodes[0] != "0000" && str_PalletCodes[0] != "0")
                        {
                            if (btn_Station1.Enabled != true)
                            {
                                btn_Station1.Enabled = true;
                                btn_Station1.BackColor = Color.OrangeRed;
                            }
                        }
                        else
                        {
                            if (btn_Station1.Enabled != false)
                            {
                                btn_Station1.Enabled = false;
                                btn_Station1.BackColor = Color.Silver;
                            }
                        }
                        if (str_PalletCodes[1] != "0000" && str_PalletCodes[1] != "0")
                        {
                            if (btn_Station2.Enabled != true)
                            {
                                btn_Station2.Enabled = true;
                                btn_Station2.BackColor = Color.OrangeRed;
                            }
                        }
                        else
                        {
                            if (btn_Station2.Enabled != false)
                            {
                                btn_Station2.Enabled = false;
                                btn_Station2.BackColor = Color.Silver;
                            }
                        }
                    }
                //}
            ...
            else
            {
                ...
                //启用str_PalletCodes[0]，当没有托盘编码的时候进入处理阶段
                //if (str_PalletCodes == null || str_PalletCodes[0] == "0" || str_PalletCodes[0] == "0000")
                //{
                    try
                    {
                        //循环调用接口
                    ...
                    else if (str_PalletCodes[0] == "0")//返回0表示PLC地址读取失败
                    {
                        lab_buhegebianma.Text = DateTime.Now.ToString() + "检查到上位机读取地址数据异常";
                    }
                    else
                    {
                        //如果不等于哨兵值，则进行加载；这里是否与起始判定重复？这里应该只要不等于“0000”即可。
                        //不等于0000则进行加载，当再次循环到这个处理逻辑的时候，在起始判断处就不会通过判定。
                        //当完成后，str_PalletCodes[0]重置为0000，则又会进行到这段处理逻辑
                        //但是还是有一个问题，当前托盘在这个工位上还未放出，则又会重新加载。
                        //哨兵值对于去除重复加载是必须的，在加上不等于0000的条件，可以排除加载0000的情况。
                        //if (str_PalletCodes[0] != palletGuard)
                        if (str_PalletCodes[0] != palletGuard&&str_PalletCodes[0] != "0000")
                        {
                            //这里加载
                            //通过验证后,加载数据
                            flag = 0;
                            DataInit(flag);
                            //加载完成后改变哨兵值，表示这个托盘已经加载过了，以后只能是有不同的才会再进行一次加载
                            palletGuard = str_PalletCodes[0];
        ...
        #region PC端托盘号加载以及工位对象初始化

        /// <summary>
        /// 托盘对应PLC地址数据加载（代替原有WCF远程服务）
        /// </summary>
        /// <param name="secondWorkStation"></param>
        /// <returns></returns>
        string[] GetPalletCodeBySQL()
        {
            //两个托盘地址
            ...
            using (XinYaDBEntities db = new XinYaDBEntities())
            {
                try
                {
                    ...
                    if (secondWorkStationInfo.WorkStationCode == "E" || secondWorkStationInfo.WorkStationCode == "F")
                    {
                        list_plcAdress = db.TB_PLCBaseAdressInfo.Where(p => p.Adress == secondWorkStationInfo.TB_PLCAdressWithWorkStationInfo.PalletCode || p.Adress == secondWorkStationInfo.TB_PLCAdressWithWorkStationInfo.ToPositCode).ToList();
                        int a1 = (int)list_plcAdress[0].Data;
                        int a2 = (int)list_plcAdress[1].Data;
                        if (a2 == 0)//等于0表示还未完成
                        {
                        ...
                        list_plcAdress = db.TB_PLCBaseAdressInfo.Where(p => p.Adress == secondWorkStationInfo.TB_PLCAdressWithWorkStationInfo.PalletCode2 || p.Adress == secondWorkStationInfo.TB_PLCAdressWithWorkStationInfo.ToPositCode2).ToList();
                        int b1 = (int)list_plcAdress[0].Data;
                        int b2 = (int)list_plcAdress[1].Data;
                    ...
                    {
                        list_plcAdress = db.TB_PLCBaseAdressInfo.Where(p => p.Adress == secondWorkStationInfo.TB_PLCAdressWithWorkStationInfo.PalletCode).ToList();
                        int a1 = (int)list_plcAdress[0].Data;
                        palletCodes[0] = IntToStringfor4Length(a1);
                        return palletCodes;
                    }
                ...
                {
                    LogExecute.WriteExceptionLog("查询托盘地址数据", ex);
                    //出现异常返回空
                    return null;
                }
            }
        ...
        #region 工作台按钮单击事件、完成事件

        /// <summary>
        /// 1号工作台加载参数信息，对应string[0]
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btn_Station1_Click(object sender, EventArgs e)
        {
            //正常情况下enable为false不会触发，这里加个验证
            if (str_PalletCodes == null | str_PalletCodes[0] == "0" || str_PalletCodes[0] == "0000")
            {
                MessageBox.Show("没有找到要加载的信息!");
            }
            else
            {
                ...
                if (secondWorkStationInfo.WorkStationCode == "F" && verifyModel == "true")
                {
                    using (XinYaDBEntities db = new XinYaDBEntities())
                    {
                        ...
                        {
                            //根据托盘条码找任务
                            string temp = str_PalletCodes[0];
                            var wm = db.TB_WorkMain.First(p => p.PalletCode == temp && p.Finished == "0");
        ...
        /// <summary>
        /// 2号工作台加载参数信息，对应string[1]
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btn_Station2_Click(object sender, EventArgs e)
        {
            //正常情况下enable为false不会触发，这里加个验证
            if (str_PalletCodes == null | str_PalletCodes[1] == "0" || str_PalletCodes[1] == "0000")
            {
                MessageBox.Show("没有找到要加载的信息!");
            }
            else
            {
                ...
                if (secondWorkStationInfo.WorkStationCode == "F" && secondWorkStationInfo.WorkStationCode == "true")
                {
                    using (XinYaDBEntities db = new XinYaDBEntities())
                    {
                        try
        ...
        /// <summary>
        /// 任务完成提交（该函数未写完10.6）（H和D工位的需要迁移）（11.30对该事件重写）
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            if (workMain == null)
            {
                MessageBox.Show("当前没有任何数据可用于提交！或者这条任务已经提交！");
                return;
            }
            //再次提醒员工比对任务托盘信息
            if (MessageBox.Show("当前托盘号为" + workMain.PalletCode + ",一旦提交之后便不能更改，是否确认完成该托盘所在的任务？", "注意", MessageBoxButtons.YesNo) == DialogResult.No)
            {
                return;
            }
            ...
            {
                if (string.IsNullOrEmpty(txt_buhegebianma.Text))//如果不合格，那么需要一个不合格的编码
                {
                    MessageBox.Show("您已经勾选了不合格，请为不合格选择一个异常编码后，再继续。");
                    return;
                }
            }
            ...
            #region 当前工位workDtlForEachStation处理(完成)、QC记录处理、20150107托盘放出验证
            ...
            using (XinYaDBEntities db = new XinYaDBEntities())
            {
                ...
                //先检查该工位是否存在记录，存在的话更新workendtime,不存在新建一条记录，时间写入workendtime
                TB_WorkDtlForEachStation a;
                try
                {
                    //a = db.TB_WorkDtlForEachStation.First(p => p.WorkMainID == workMain.WorkID&&p.SecondWorkStationID==secondWorkStationId);
            ...
            #endregion 当前工位workDtlForEachStation处理(完成)、QC记录处理

            if (flagForComplete==0)
            {
                #region PLC地址处理（地址写入异常必须停止操作，这样没能写地址进去，托盘也不可能放得出来，程序会停在这里，可能原因为与PLC通讯故障，除非解决故障，否则接下来怎么办我也不知道了~）
                ...
                switch (secondWorkStationInfo.WorkStationCode)
                {
                    ...
                    case "E":
                        ...
                        try
                        {
                            resulte = service1Client.WritePLCAdressOnSESF(secondWorkStationId, Convert.ToInt16(workMain.PalletCode), exception);
                        }
                        catch
                        {
                        ...
                        switch (resulte)
                        {
                            ...
                            case 4:
                                MessageBox.Show("注意：当前托盘：" + workMain.PalletCode + "并不在当前工位存在！请再次核实托盘条码是否正确！");
                                return;
                            ...
                            default:
                                ...
                                lab_CurrentStatus.Text = DateTime.Now.ToString() + "无";
                                break;
                        }
                        break;
