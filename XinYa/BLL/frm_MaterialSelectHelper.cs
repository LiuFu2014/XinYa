...
namespace XinYa.BLL
{
    public partial class frm_MaterialSelectHelper : Form
    {
        ...
        /// <summary>
        /// 1为合格率，2为关键零部件信息绑定,3为生产计划,4为关键零部件查询
        /// </summary>
        ...
        private void btn_Confirm_Click(object sender, EventArgs e)
        {
            ...
            if (flag==2)
            {
                frm_KeyComponentsBanding com = (frm_KeyComponentsBanding)this.Tag;
                //回传前先清空，以免数据干扰
                //com.txt_MatSelect.Text = "";
                //循环遍历
                if (dgv_MatSelect.Rows.Count != 0 && dgv_MatSelect.DataSource != null)
                {
                    for (int i = 0; i < dgv_MatSelect.Rows.Count; i++)
                    {
                        if ((dgv_MatSelect.Rows[i].Cells["c_Select"].Value == null ? "false" : dgv_MatSelect.Rows[i].Cells["c_Select"].Value.ToString()) == "true")
                        {
                            com.txt_MatType.AppendText(dgv_MatSelect.Rows[i].Cells["c_TypeCode"].Value.ToString() + ";");
                        }
                    }
                }
            }
            if (flag==3)
            {
                frm_ProductionPlan com = (frm_ProductionPlan)this.Tag;
                //回传前先清空，以免数据干扰
                //com.txt_MatSelect.Text = "";
                //循环遍历
                if (dgv_MatSelect.Rows.Count != 0 && dgv_MatSelect.DataSource != null)
                {
                    for (int i = 0; i < dgv_MatSelect.Rows.Count; i++)
                    {
                        if ((dgv_MatSelect.Rows[i].Cells["c_Select"].Value == null ? "false" : dgv_MatSelect.Rows[i].Cells["c_Select"].Value.ToString()) == "true")
                        {
                            com.txt_MatSelect.AppendText(dgv_MatSelect.Rows[i].Cells["c_TypeCode"].Value.ToString() + ";");
                        }
                    }
                }
            }
            if (flag==4)
            {
                frm_KeyComponentsSer kms = (frm_KeyComponentsSer)this.Tag;
                //回传前先清空，以免数据干扰
                //com.txt_MatSelect.Text = "";
                //循环遍历
                if (dgv_MatSelect.Rows.Count != 0 && dgv_MatSelect.DataSource != null)
                {
                    for (int i = 0; i < dgv_MatSelect.Rows.Count; i++)
                    {
                        if ((dgv_MatSelect.Rows[i].Cells["c_Select"].Value == null ? "false" : dgv_MatSelect.Rows[i].Cells["c_Select"].Value.ToString()) == "true")
                        {
                            kms.txt_MatType.AppendText(dgv_MatSelect.Rows[i].Cells["c_TypeCode"].Value.ToString() + ";");
                        }
                    }
                }
            }
